/*
  # Add voting status check functions

  1. New Functions
    - `check_student_vote_status` - Check if a student has voted in an election
    - `get_election_results` - Get vote counts and percentages for an election
  
  2. Security
    - Grant execute permissions to authenticated users
    - Functions use RLS policies from underlying tables
*/

-- Function to check if a student has voted in a specific election
CREATE OR REPLACE FUNCTION public.check_student_vote_status(
    student_uuid uuid,
    election_uuid uuid
)
RETURNS TABLE (
